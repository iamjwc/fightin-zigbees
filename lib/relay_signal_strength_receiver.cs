...
{
  public class RelaySignalStrengthReceiver : SignalStrengthReceiver
  {
    public RelaySignalStrengthReceiver(Xbee xb) : base(xb){}
    ...
    public override List<uint>[] get_signal_strengths(float time, int max_num_sig_str)
    {
      ...
      do
      {
        ...
        uint address = packet.get_relay_address();
        byte sig_str = packet.get_relay_signal_strength();

        /*TODO: We need to put an if statement around all of this so that
          TODO: it only takes packets from the relay XBee, so I have to get its
          TODO: address from Jeff*/
